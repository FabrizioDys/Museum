...
public unsafe class DracoMeshLoader 
{
    ...
    public int LoadMeshFromAsset(string assetName, ref List<Mesh> meshes)
    {
        ...
        byte[] encodedData = asset.bytes; //stream di bit da draco
        ...
        return ConvertDracoMeshToUnity(encodedData, ref meshes); //passo lo stream insieme al riferimento alla lista di mesh da far vedere
    }

    public int LoadMeshFromInternet(ref List<Mesh> meshes, byte[] ta)
    {
         
        byte[] encodedData = ta; //stream di bit da draco
        Debug.Log(encodedData.Length.ToString());
        if (encodedData.Length == 0)
        {
            Debug.Log("Didn't load encoded data!");
